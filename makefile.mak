...
# is equivalent to 'make p2', due to the definition of PROGRAM above).

${PROGRAM}:     getword.o p2.o
                ${CC} -o p2 getword.o p2.o
# the action on the above line calls gcc (actually, it calls the linker, since
# we are dealing with .o object modules), links them, and renames a.out to p2 .
# The make program first checks if the two files getword.o and p2.o are up to
# date (that is, if their time stamps are younger than getword.c and p2.c,
# respectively).  If (say) p2.o is not up to date, then make looks for guidance
# about how to create p2.o, which is listed below:

p2.o:           getword.h

# p2.c could be listed as a dependency above, but 'make' is smart enough to know
# that p2.o depends on p2.c, so we don't really have to list it.
...
# to know that p2.o depends on p2.c, but there's no easy way for it to figure
# out that p2.c also depends on getword.h -- so we have to explicitly say so.
